#include "opengl-renderer-context-windows.h"
...
std::string fbr::opengl::windows::ContextOpenGLWindows::RegisterTemporaryClass()
{
	...
	wc.lpszClassName = L"opengl_temp_window_class";
...
{
	HWND handleTMP = CreateWindow(std::wstring(classname.begin(), classname.end()).c_str(), L"", WS_POPUP, 0, 0, 300, 300, 0, 0, m_executableInstance, nullptr);
...
void fbr::opengl::windows::ContextOpenGLWindows::DeleteTemporaryObjects(std::string classname, HWND handleTMP, HDC devTMP, HGLRC ctxTMP)
{
	...
	if (!classname.empty())
	{
		UnregisterClass(std::wstring(classname.begin(), classname.end()).c_str(), m_executableInstance);
